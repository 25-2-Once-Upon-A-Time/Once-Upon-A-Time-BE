...
import pproject.once_upon_a_time.domain.member.domain.Member; // Add this import
...
public class StoryUpdateService {
     ...
     */
    @Transactional
    public void finalizeStory(Long storyId, AiResponse aiResponse) {
        Story story = storyRepository.findById(storyId)
                .orElseThrow(() -> new IllegalArgumentException("Story not found"));

        // 1. AI 응답으로 Story 정보 업데이트
        MetadataDto metadata = aiResponse.metadata();
        ...
        story.updateThumbnailUrl(aiResponse.thumbnailUrl()); // 썸네일 URL 업데이트
        story.updateCompletedAt(LocalDateTime.now()); // 완료 시간 설정

        // 2. AI 응답으로 Script 엔티티 리스트 생성 및 저장
        final Story finalStory = story;
        List<Script> scripts = aiResponse.scripts().stream()
                .map(dto -> Script.builder()
                        .story(finalStory)
                        .seq(dto.seq())
                        .role(dto.role())
                        .text(dto.text())
                        .audioFilePath(dto.audioFilePath()) // audioFilePath 추가
                        .build())
